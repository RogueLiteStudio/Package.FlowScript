...
namespace Flow
{
    public static class GraphPasteUtil
    {
        ...
        public static void Paste(FlowGraphEditor editor, FlowGraphCopyData copyData)
        {
            sGUIDMap.Clear();
            if (!CheckPaste(editor, copyData))
                return;
            GraphEditorUtil.RegisterUndo(editor.Graph.Owner, "Paste Nodes");
        ...
        private static FlowNode PasteNode(FlowSubGraph subGraph, FlowCopyNode copyData)
        {
            ...
            {
                return subGraph.Owner.FindNode(newGUID);
            }
        ...
        private static FlowSubGraph PasteSubGraph(FlowGraph graph, FlowCopySubGraph copyData)
        {
            ...
            if (!sGUIDMap.TryGetValue(copyData.BindNode, out var nodeGUID))
                return null;
            var bindNode = graph.FindNode(nodeGUID);
            if (bindNode == null)
                return null;
            var subGraph = GraphCreateUtil.CreateSubGraph(graph, copyData.AllowStageNode);
            sGUIDMap.Add(copyData.GUID, subGraph.GUID);
            GraphCreateUtil.BindSubGraphToNode(subGraph, bindNode);
